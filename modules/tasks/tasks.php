...
$EndDate = getProjectSubState('TaskDates', 'End', $db_start_date[0]['project_finish_date'] );
...
$allowedProjects[] = $pid;
// Non ci interessa richiamere allowedProjects perché una persona può non avere la capability view project ma view task e quindi deve visualizzarne i tasks!
...
$task_status = intval( getProjectSubState('Tasks', 'inactive' ) );
...
<form id="task_list_options" name='task_list_options' method='post'
	...
	name="display_option" value="<?php echo $display_option;?>" /> <input
	type="hidden" name="roles" value="<? echo $roles?>" />
...
<table border='0' cellpadding='1' cellspacing='3' align="center">
	<tr>
		...
		<table border="0" cellspacing="0">
			<tr>
				<td class="tab_setting_title" rowspan="2"><?php echo $AppUI->_('Show');?>:</td>
				<td class="tab_setting_item"><input type='checkbox'
					id="show_incomplete" name='show_incomplete'
					<?php echo $showIncomplete ? 'checked="checked"' : '';?> /> <label
					for="show_incomplete"><?php echo $AppUI->_('Incomplete tasks only'); ?></label>
				</td>
			</tr>
			<tr>
				<td><input type='checkbox' id='show_mine' name='show_mine'
				<?php echo $showMine ? 'checked="checked"' : '';?> /> <label
					for="show_mine"><?php echo $AppUI->_('My tasks only'); ?></label></td>
			</tr>
			<tr>
				<td class="tab_setting_title"><?php echo $AppUI->_('Explode Tasks').": ";?>
				</td>
		...
		<table border="0" cellspacing="0">
			<tr>
				<td class="tab_setting_title"><?php echo $AppUI->_('From');?>:</td>
				<td><input type="hidden" name="sdate"
					value="<?php echo $start_date->format( FMT_TIMESTAMP_DATE );?>" />
				<input type="text" disabled="disabled" class="text"
					name="show_sdate" value="<?php echo $start_date->format( $df );?>"
					size="12"
					onchange='document.task_list_options.sdate.value=this.value; validateDate(this);' />
				<a href="#" onclick="popCalendar('sdate')"><img
					src="./images/calendar.gif" width="24" height="12" alt=""
					border="0"></a></td>
			</tr>
			<tr>
				<td class="tab_setting_title"><?php echo $AppUI->_( 'To' );?>:</td>
				<td align="left" nowrap="nowrap"><input type="hidden" name="edate"
					value="<?php echo $end_date->format( FMT_TIMESTAMP_DATE );?>" /> <input
					type="text" disabled="disabled" class="text" name="show_edate"
					value="<?php echo $end_date->format( $df );?>" size="12"
					onchange='document.task_list_options.edate.value=this.value; validateDate(this);' />
				<a href="#" onclick="popCalendar('edate')"><img
					src="./images/calendar.gif" width="24" height="12" alt=""
					border="0"></a></td>
			</tr>
			<tr>
				<td class="tab_setting_title"><?php echo $AppUI->_( 'Reset' );?>:</td>
				<td><a href="#" onclick="showFullProject()"><img
					src="./images/calendar2.gif" alt="Show Whole Project"
					title="Show Whole Project" border="0"></a></td>
			</tr>
		</table>
		</td>
...
<table width="100%" border="0" cellpadding="2" cellspacing="1"
	...
	<tr>
		<td
			colspan="<?php echo $dPconfig['direct_edit_assignment'] ? $cols-4 : $cols; ?>">
		<table width="100%" border="0">
			<tr>
				...
				if ($viewPrl) {?> <a
					href="./index.php?m=projects&a=view&project_id=<?php echo $k;?>"> <?php } ?>
				<span style='color:
				<?php
					echo bestColor( @$p["project_color_identifier"] ); 
				?>;
					text-decoration:none;'><strong> <?php echo @$p["group_name"].' :: '.@$p["project_name"];?>
				</strong></span> <?php if ($viewPrl) { ?> </a> <?php } ?></td>
				<td width="<?php echo (100 - intval($pp));?>%"><?php echo ($pp);?>%
				</td>
			</tr>
		</table>
		</td>
	...
	setProjectSubState("Tasks", "PersonsRoles", $roles);
