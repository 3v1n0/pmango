...
define('CONSTRAIN_MIDDLESTARTSIDE',7);
define('CONSTRAIN_MIDDLEEND',8);
define('CONSTRAIN_MIDDLEMIDDLE',9);
define('CONSTRAIN_ENDSIDESTARTSIDE',10);
define('CONSTRAIN_ENDSIDEMIDDLE',11);
                    ...
                    if( count($c1) == 4 && count($c2 ) == 4) {
                        switch( $this->iObj[$i]->constraints[$k]->iConstrainType ) {
                            case CONSTRAIN_ENDSTART:
                                ...
                                $link->SetPath(3);
                                break;
                            case CONSTRAIN_ENDSIDESTARTSIDE:
                            ...
                            case CONSTRAIN_STARTSTART:
                             ...
                             case CONSTRAIN_ENDMIDDLE:
                                ...
                                $link->SetPath(0);
                                break;
                             case CONSTRAIN_ENDSIDEMIDDLE:
                                if( $c1[1] < $c2[1] ) {
                                	$link = new GanttLink($c1[2],$c1[1]+($c1[3]-$c1[1])/2,$c2[0]+($c2[2]-$c2[0])/2,$c2[1]);
                                } else {
                                	$link = new GanttLink($c1[2],$c1[1]+($c1[3]-$c1[1])/2,$c2[0]+($c2[2]-$c2[0])/2,$c2[3]);
                                }
                                $link->SetPath(5);
                                break;
                             case CONSTRAIN_MIDDLESTART:
                                ...
                                $link->SetPath(0);
                                break;
                             case CONSTRAIN_MIDDLESTARTSIDE:
                                if( $c1[1] < $c2[1] ) {
                                    $link = new GanttLink($c1[0]+($c1[2]-$c1[0])/2,$c1[3],$c2[0],$c2[1]+($c2[3]-$c2[1])/2);
                                }
                                else {
            ...
            if( $x2 > $x1 ) {
                switch ( $this->iPathType  ) {
                    ...
                    case 5:
                    	...
                    	break;
                    case 6:
                    	if ($x2 > $x1 + $this->iPathExtend) {
		                    $c = array($x1,$y1,
		                            $x1,$y2,
		                            $x2,$y2);
                    	} else {
                    		$c = array($x1,$y1,
		                            $x1,$y2-$this->iPathExtend,
		                            $x1-$this->iPathExtend,$y2-$this->iPathExtend,
		                            $x1-$this->iPathExtend,$y2,
		                            $x2,$y2);
                    	}
